// DecompilerFi decompiler from Assembly-CSharp-firstpass.dll class: ControlFreak2.Internal.JoystickState
...
namespace ControlFreak2.Internal
{
	public class JoystickState
	{
		...
		public void Update()
		{
			...
			dir = ((num2 > num4) ? CFUtils.DirFromAngleEx(ang, false, dirLastNonNeutral4, config.angularMagnet) : ((num2 > config.digitalLeaveThresh) ? dir : Dir.N));
			float num5 = (dirLastNonNeutral8 == Dir.N) ? (0.5f * (config.digitalEnterThresh + config.digitalLeaveThresh)) : config.digitalEnterThresh;
			dir2 = ((num3 > num5) ? CFUtils.DirFromAngleEx(ang, true, dirLastNonNeutral8, config.angularMagnet) : ((num3 > config.digitalLeaveThresh) ? dir2 : Dir.N));
			if (dir != 0)
			{
