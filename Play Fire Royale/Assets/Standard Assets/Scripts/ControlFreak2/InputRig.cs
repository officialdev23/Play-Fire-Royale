// DecompilerFi decompiler from Assembly-CSharp-firstpass.dll class: ControlFreak2.InputRig
	...
	public class InputRig : ComponentBase, IBindingContainer
	{
		...
		private class EmulatedTouchState
		{
			...
			public void Start(Vector2 pos)
			{
				...
				touch.OnTouchStart(pos, pos, 0f,  false,  false, 1f);
			}
		...
		public class AutomaticInputConfig : NamedConfigElement
		{
			...
			public override void Update(InputRig rig)
			{
				...
				targetBinding.Sync( true, rig, skipIfTargetIsMuted: true);
			}
		}
		...
		public class TiltConfig : IBindingContainer
		{
			...
			public TiltConfig(InputRig rig)
			{
				...
				disablingConditions.mobileModeRelation = DisablingConditionSet.MobileModeRelation.EnabledOnlyInMobileMode;
				rollBinding = new AxisBinding("Horizontal",  false);
				pitchBinding = new AxisBinding("Vertical",  false);
			...
			public void Update()
			{
				...
				if (IsEnabled())
				{
					...
					{
						((digitalRollDirection >= 0) ? rollRightBinding : rollLeftBinding).Sync( true, rig);
					}
					if (digitalPitchDirection != 0)
					{
						((digitalPitchDirection >= 0) ? pitchForwardBinding : pitchBackwardBinding).Sync( true, rig);
					}
				}
			}
		...
		public class ScrollWheelState : IBindingContainer
		{
			...
			{
				this.rig = rig;
				vertScrollDeltaBinding = new ScrollDeltaBinding("Mouse ScrollWheel",  true);
				horzScrollDeltaBinding = new ScrollDeltaBinding("Mouse ScrollWheel Secondary",  true);
			}
		...
		public class MouseConfig : IBindingContainer
		{
			...
			{
				this.rig = rig;
				horzDeltaBinding = new AxisBinding("Mouse X",  false);
				vertDeltaBinding = new AxisBinding("Mouse Y",  false);
			}
		...
		public void OnActivateRig()
		{
			...
			{
				ShowOrHideTouchControls( true, skipAnim: false);
			}
		}
		...
		public void OnDisactivateRig()
		{
			...
			{
				ShowOrHideTouchControls( false, skipAnim: false);
			}
		}
		...
		private void BasicConstructor()
		{
			...
			anyGamepad.digiFaceDownBinding.AddAxis().SetAxis("Fire1",  true);
			anyGamepad.digiFaceRightBinding.enabled = true;
			anyGamepad.digiFaceRightBinding.AddAxis().SetAxis("Jump",  true);
			anyGamepad.digiFaceLeftBinding.enabled = true;
			anyGamepad.digiFaceUpBinding.enabled = true;
			anyGamepad.digiFaceUpBinding.AddAxis().SetAxis("Fire3",  true);
			anyGamepad.digiR1Binding.enabled = true;
			anyGamepad.digiR1Binding.AddAxis().SetAxis("Fire1",  true);
			anyGamepad.digiR2Binding.enabled = true;
			anyGamepad.digiR2Binding.AddAxis().SetAxis("Fire1",  true);
			anyGamepad.digiL1Binding.enabled = true;
			anyGamepad.digiL1Binding.AddAxis().SetAxis("Fire2",  true);
			anyGamepad.digiL2Binding.enabled = true;
			anyGamepad.digiL2Binding.AddAxis().SetAxis("Fire2",  true);
