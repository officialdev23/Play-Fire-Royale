// DecompilerFi decompiler from Assembly-CSharp-firstpass.dll class: ControlFreak2.SuperTouchZone
	...
	public class SuperTouchZone : TouchControl
	{
		...
		protected class MultiFingerTouch
		{
			...
			public void Start(FingerState[] fingers, Vector2 startScreenPos, Vector2 startOrientedPos, bool quietMode, float quietAlreadyElapsed)
			{
				...
				if (!quietMode)
				{
					touchScreen.OnTouchStart(startScreenPos, startScreenPos, 0f, controlledByMouse,  false, 1f);
					touchOriented.OnTouchStart(startOrientedPos, startOrientedPos, 0f, controlledByMouse,  false, 1f);
				}
			}
			...
			public void StartOfficially()
			{
				...
				{
					quietMode = false;
					touchScreen.OnTouchStart(posStartScreen, posCurScreen, quietModeElapsed, controlledByMouse,  false, 1f);
					touchOriented.OnTouchStart(posStartOriented, posCurOriented, quietModeElapsed, controlledByMouse,  false, 1f);
				}
			}
			...
			private void StartDrivingControl(TouchControl c)
			{
				...
				{
					drivingTouch.Start(touchScreen.GetStartPos(), touchScreen.GetCurPosRaw(), zone.GetCamera(),  false,  false, 1f);
		...
		private class EmulatedFingerSystem
		{
			...
			public void StartTouches(int num, Vector2 center, bool isMouse)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					emulatedFingerObject.Start(emulatedFingerObject.emuPos, emulatedFingerObject.emuPos, zone.GetCamera(), isMouse,  false, 1f);
			...
			public void Update()
			{
				...
				else
				{
					...
					if (curFingerNum == 0)
					{
						...
						StartTouches(num4, zone.GetScreenSpaceCenter(zone.GetCamera()),  false);
					}
			...
			public void DrawMarkers()
			{
				...
				if (curFingerNum > 1)
				{
					...
					if (key && flag)
					{
						...
						num2 *= Mathf.Clamp01(Mathf.Sin(num4 * 2f * (float)Math.PI));
						num3 *= Mathf.Clamp01(Mathf.Sin((num4 + 0.5f) * 2f * (float)Math.PI));
					}
					float num5 = 128f;
			...
			private Vector2 GetFingerPos(int fingerId)
			{
				...
				float f = GetFingerAngle(fingerId) * ((float)Math.PI / 180f);
		...
		protected override void OnUpdateControl()
		{
			...
			if (PinchActive())
			{
				...
				{
					spreadDigitalBinding.Sync( true, base.rig);
				}
				else if (GetPinchDist() < thresholds.pinchDigitalThreshPx)
				{
					pinchDigitalBinding.Sync( true, base.rig);
				}
			}
			if (TwistActive())
			{
				...
				{
					twistRightDigitalBinding.Sync( true, base.rig);
				}
				if (GetTwist() < thresholds.twistDigitalThresh)
				{
					twistLeftDigitalBinding.Sync( true, base.rig);
				}
			}
		}
